...
Notes:
- **Page**, **layout**, and **parallel-route group** (`@slot`) loaders all support `defer`. A deferred group renders its own `loading.tsx` while its data streams in, independently of the rest of the page — mark the group's loader `type: 'defer'` and add a `loading.tsx` beside it (see the `@slot` example in [Routing](./routing.md)). Layouts have no `loading.tsx`/`error.tsx` of their own, so a deferred layout loader falls back to the **route's** `loading.tsx`/`error.tsx` (none → no fallback/boundary, the resource error just propagates uncaught).
- Marking a layout loader `type: 'defer'` changes its failure contract, not just its timing: a **sequential** layout loader never throws (it yields the error sentinel above, siblings still render), but a **deferred** layout loader throws toward `error.tsx` on rejection, same as a deferred page/group loader.
